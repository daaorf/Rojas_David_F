...
### 1. Fuentes fijas:
### Instalaciones de ubicación fija que tienen como finalidad desarrollar operaciones o procesos industriales de manufactura

### 2. Fuentes móviles que circulan por carretera:
### Vehículos con motores de combustión y similares que abarca una variedad de vehículos como: tractocamiones, autobuses, camiones, automóviles, motocicletas

### 3. Fuentes móviles que no circulan por carretera:
### Vehículos con motores de combustión y similares, comprenden: aviones, helicópteros, ferrocarriles, tranvías, embarcaciones, equipo y maquinarias.

### 4. Fuentes de área:
### Fuentes pequeñas, numerosas y dispersas; abarca los comercios, servicios, viviendas, entre otros establecimientos.

### 5. Fuentes naturales:
### Emisiones biogénicas, las emisiones del suelo y la erosión eólica
...
### 5. Partículas igual o menores a 10 micrómetros (PM10)

### 6. Partículas igual o menores a 2.5 micrómetros (PM2.5)
...
## 2. Importación y modificación de base de datos
...
### 2.3 Combinación de población y geografía de cada estado
...
### 2.5 Mapa de México y densidad de población por estado
```{r message=FALSE, warning=FALSE}
...
### 2.6 Contaminación
### 2.6.1 Contaminación por estado
...
### 2.7 Mapas de México y contaminantes por estado
...
### Compuestos orgáinicos volátiles
```{r message=FALSE, warning=FALSE}
ggplot(mexco) +
...
### 3.1 Uniones finales para los análisis
...
### 3.2 Correlación entre densiad y contaminante

### 3.2.1 Modificación de la tabla para realizar los análisis
...
### 3.2.2 Correlación entre densidad y tipo de contaminante
```{r message=FALSE, warning=FALSE}
cord <- bd_prueba %>%
  #Dentro de bd_prueba...
...
### 3.3  Relación entre contaminante y tipo de fuente
...
```
##### Fuentes más relacionada con SO2: Fuentes Fijas
...
```
##### Fuentes más relacionada con CO: Fuentes Móviles Carreteros
...
ggplot(NOx, aes(x = Tipo_de_Fuente, y = Emision, fill = Tipo_de_Fuente)) +
  ...
  labs(title = "Emisiones de NOx por tipo de fuente",
       ...
       x = "Tipo de fuente")
```
##### Fuentes más relacionada con NOx: Fuentes Móviles Carreteros
...
ggplot(COV, aes(x = Tipo_de_Fuente, y = Emision, fill = Tipo_de_Fuente)) +
  ...
  labs(title = "Emisiones de COV por tipo de fuente",
       ...
       x = "Tipo de fuente")
```
##### Fuentes más relacionada con COV: Fuentes Naturales
...
ggplot(PM_010, aes(x = Tipo_de_Fuente, y = Emision, fill = Tipo_de_Fuente)) +
  ...
  labs(title = "Emisiones de PM10 por tipo de fuente",
       ...
       x = "Tipo de fuente")
```
##### Fuentes más relacionada con PM10: Fuentes de Área y Fuentes Fijas
...
ggplot(PM_2_5, aes(x = Tipo_de_Fuente, y = Emision, fill = Tipo_de_Fuente)) +
  ...
  labs(title = "Emisiones de PM2.5 por tipo de fuente",
       ...
       x = "Tipo de fuente")
```
##### Fuentes más relacionada con PM2.5: Fuentes Fijas
...
```
##### Fuentes más relacionada con NH3, Fuentes de Área
...
#### Con esto podemos resolver la 1° pregunta: ¿Qué tipos de fuentes contribuyeron más a la emisión de cada tipo de contaminate? Pues cada una o más emiten ciertos contaminantes en mayor medida que otros, por ejemplo, las fuentes naturales emiten más COV que las demás, mientras que el Amoniaco (NH3) es emitido principalmente por medio de fuentes de área.

### 3.4 Relación entre estados y contaminantes
### 3.4.1 Regiones
```{r}
